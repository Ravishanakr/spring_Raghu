...
public class AppSecurity {
	...
	SecurityFilterChain securityConfig(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(
				req -> req.requestMatchers("/", "/home").permitAll().requestMatchers("/admin").hasAuthority("ADMIN")
	...
	@Bean
	UserDetailsService users(DataSource source) {
		...
		UserDetails u1 = User.withUsername("Ravi").password("Ravi").authorities("ADMIN").build();
		UserDetails u2 = User.withUsername("Shankar").password("Shankar123").authorities("CUSTOMER").build();
